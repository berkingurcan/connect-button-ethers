...
const Connect = () => {
    ...
    async function connectToMetamask(){
        if (window.ethereum) {
            let provider;
            try {
                provider = new ethers.providers.Web3Provider(window.ethereum, 4002);
            } catch (err) {
            ...
            const balanceOfAddressLast = parseInt((balanceOfAddress._hex).toString(16),16)/10**18
            setIsConnected(true)
  ...
  return (
    ...
    <Button onClick={connectToMetamask} variant="contained">
        {isConnected ? <Typography>{btnAddress.slice(0,8) + "...  "}[{accBalance} FTM]</Typography> : "Connect"}
    </Button>
    </div>
  )
}
